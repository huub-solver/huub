...
Inside the callback, you can query variable values using the `val` method from the `IntValuation` trait.
...
2. **Branching:** If domains cannot be reduced further, the solver picks an unassigned variable and tries assigning it a value from its domain.
